...
func (suite *ServiceRequestsAPISuite) Test_TransitionStatus_NewToAssigned_Returns200() {
	// ARRANGE
	suite.srDbMock.
		On("FindDocument", mock.Anything, "sr-1").
		Return(sampleServiceRequestDoc(), nil)
	...
	body := `{"status":"ASSIGNED","assignedTo":"tech-1"}`
	ctx, rec := suite.makeCtx("PATCH", "/v1/service-requests/sr-1/status", body, gin.Params{
		{Key: "requestId", Value: "sr-1"},
	})

	// ACT
	suite.sut.TransitionServiceRequestStatus(ctx)
...
func (suite *ServiceRequestsAPISuite) Test_TransitionStatus_InvalidTransition_Returns422() {
	// ARRANGE
	suite.srDbMock.
		On("FindDocument", mock.Anything, "sr-1").
		Return(sampleServiceRequestDoc(), nil) // status=NEW

	body := `{"status":"CLOSED","resolutionNote":"Fixed"}`
	ctx, rec := suite.makeCtx("PATCH", "/v1/service-requests/sr-1/status", body, gin.Params{
		{Key: "requestId", Value: "sr-1"},
	})
...
func (suite *ServiceRequestsAPISuite) Test_TransitionStatus_AssignedMissingAssignedTo_Returns400() {
	// ARRANGE
	suite.srDbMock.
		On("FindDocument", mock.Anything, "sr-1").
		Return(sampleServiceRequestDoc(), nil) // status=NEW
...
func (suite *ServiceRequestsAPISuite) Test_TransitionStatus_ClosedMissingResolutionNote_Returns400() {
	// ARRANGE
	inProgressSr := sampleServiceRequestDoc()
	inProgressSr.Status = IN_PROGRESS
	suite.srDbMock.
		On("FindDocument", mock.Anything, "sr-1").
		Return(inProgressSr, nil)
...
func (suite *ServiceRequestsAPISuite) Test_TransitionStatus_NotFound_Returns404() {
	// ARRANGE
	suite.srDbMock.
		On("FindDocument", mock.Anything, "missing-id").
		Return(nil, db_service.ErrNotFound)
	...
	body := `{"status":"ASSIGNED","assignedTo":"tech-1"}`
	ctx, rec := suite.makeCtx("PATCH", "/v1/service-requests/missing-id/status", body, gin.Params{
		{Key: "requestId", Value: "missing-id"},
	})
...
func (suite *ServiceRequestsAPISuite) Test_TransitionStatus_AssignedToNew_ClearsAssignedTo() {
	// ARRANGE
	assignedTo := "tech-1"
	assignedSr := sampleServiceRequestDoc()
	assignedSr.Status = ASSIGNED
	assignedSr.AssignedTo = &assignedTo
	suite.srDbMock.
		On("FindDocument", mock.Anything, "sr-1").
		Return(assignedSr, nil)
	suite.srDbMock.
	...
	var updatedDoc *ServiceRequestDoc
	for _, call := range suite.srDbMock.Calls {
		if call.Method == "UpdateDocument" {
			updatedDoc = call.Arguments.Get(2).(*ServiceRequestDoc)
			break
		}
	}
	suite.Require().NotNil(updatedDoc)
	suite.Nil(updatedDoc.AssignedTo)
}
