...
func (suite *EquipmentAPISuite) Test_MoveEquipment_ValidMove_Returns200() {
	// ARRANGE
	suite.equipmentDbMock.
		On("FindDocument", mock.Anything, "eq-1").
		Return(sampleEquipmentDoc(), nil)
	suite.locationDbMock.
		On("FindDocument", mock.Anything, "new-location").
		Return(&Location{Id: "new-location", Building: "B", Floor: "2", Department: "Radiology", Room: "202"}, nil)
	suite.equipmentDbMock.
		On("UpdateDocument", mock.Anything, "eq-1", mock.Anything).
		Return(nil)

	body := `{"locationId":"new-location"}`
	ctx, rec := suite.makeCtx("PATCH", "/v1/equipment/eq-1/location", body, gin.Params{
		{Key: "equipmentId", Value: "eq-1"},
	})

	// ACT
	suite.sut.MoveEquipment(ctx)
...
func (suite *EquipmentAPISuite) Test_MoveEquipment_EquipmentNotFound_Returns404() {
	// ARRANGE
	suite.equipmentDbMock.
		On("FindDocument", mock.Anything, "missing-id").
		Return(nil, db_service.ErrNotFound)
	...
	body := `{"locationId":"new-location"}`
	ctx, rec := suite.makeCtx("PATCH", "/v1/equipment/missing-id/location", body, gin.Params{
		{Key: "equipmentId", Value: "missing-id"},
	})
...
func (suite *EquipmentAPISuite) Test_MoveEquipment_LocationNotFound_Returns400() {
	// ARRANGE
	suite.equipmentDbMock.
		On("FindDocument", mock.Anything, "eq-1").
		Return(sampleEquipmentDoc(), nil)
	...
	body := `{"locationId":"missing-location"}`
	ctx, rec := suite.makeCtx("PATCH", "/v1/equipment/eq-1/location", body, gin.Params{
		{Key: "equipmentId", Value: "eq-1"},
	})
...
func (suite *EquipmentAPISuite) Test_MoveEquipment_MissingLocationId_Returns400() {
	// ARRANGE
	suite.equipmentDbMock.
		On("FindDocument", mock.Anything, "eq-1").
		Return(sampleEquipmentDoc(), nil)
...
func (suite *EquipmentAPISuite) Test_ListEquipmentServiceRequests_Found_Returns200() {
	// ARRANGE
	suite.equipmentDbMock.
		On("FindDocument", mock.Anything, "eq-1").
		Return(sampleEquipmentDoc(), nil)
	...
	suite.serviceRequestDbMock.
		...
		Return([]*ServiceRequestDoc{{Id: "sr-1", EquipmentId: "eq-1", Status: NEW}}, nil)

	ctx, rec := suite.makeCtx("GET", "/v1/equipment/eq-1/service-requests", "", gin.Params{
		{Key: "equipmentId", Value: "eq-1"},
	})

	// ACT
	suite.sut.ListEquipmentServiceRequests(ctx)

	// ASSERT
	suite.Equal(http.StatusOK, rec.Code)
	suite.serviceRequestDbMock.AssertCalled(suite.T(), "FindDocumentsByFilterPaginated", mock.Anything, mock.Anything, int64(0), int64(20))
}

func (suite *EquipmentAPISuite) Test_ListEquipmentServiceRequests_EquipmentNotFound_Returns404() {
	// ARRANGE
	suite.equipmentDbMock.
		On("FindDocument", mock.Anything, "missing-id").
		Return(nil, db_service.ErrNotFound)

	ctx, rec := suite.makeCtx("GET", "/v1/equipment/missing-id/service-requests", "", gin.Params{
		{Key: "equipmentId", Value: "missing-id"},
	})
	...
	// ASSERT
	suite.Equal(http.StatusNotFound, rec.Code)
	suite.serviceRequestDbMock.AssertNotCalled(suite.T(), "FindDocumentsByFilterPaginated", mock.Anything, mock.Anything, mock.Anything, mock.Anything)
}

func (suite *EquipmentAPISuite) Test_ListEquipmentServiceRequests_WithStatusFilter_PassesFilterToDb() {
	// ARRANGE
	suite.equipmentDbMock.
		On("FindDocument", mock.Anything, "eq-1").
		Return(sampleEquipmentDoc(), nil)
	...
	suite.serviceRequestDbMock.
		On("FindDocumentsByFilterPaginated", mock.Anything, mock.Anything, int64(0), int64(20)).
		Return([]*ServiceRequestDoc{}, nil)

	ctx, rec := suite.makeCtx("GET", "/v1/equipment/eq-1/service-requests?status=NEW", "", gin.Params{
		{Key: "equipmentId", Value: "eq-1"},
	})
	...
	var filterArg bson.D
	for _, call := range suite.serviceRequestDbMock.Calls {
		if call.Method == "CountDocumentsByFilter" {
			filterArg = call.Arguments.Get(1).(bson.D)
			break
		}
	...
	suite.Require().NotEmpty(filterArg, "expected a non-empty filter to be passed to CountDocumentsByFilter")
	found := false
	for _, e := range filterArg {
		if e.Key == "status" && e.Value == "NEW" {
			found = true
			break
		}
	}
	suite.True(found, "expected status=NEW in filter")
}
